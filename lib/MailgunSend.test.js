...
describe(MailgunSend, () => {
  ...
  describe('#send', () => {
    let mailgunSend = new MailgunSend(creds);
    ...
    describe('when send is not successful', () => {
      afterEach(() => {
        mailgunSend = new MailgunSend(creds);
      });

      it('handles network errors', async () => {
        mailgunSend = new MailgunSend({ ...creds, apiKey: 'FAIL_NETWORK' });
        await expect(mailgunSend.send(options))
          .rejects
          .toThrow(/check your network settings/i);
      });

      it('handles unauthorized errors', async () => {
        mailgunSend = new MailgunSend({ ...creds, apiKey: 'FAIL_UNAUTHORIZED' });
        await expect(mailgunSend.send(options))
          .rejects
          .toThrow(/rerun your command with -R flag to re-enter you credentials/i);
      });

      it('handles any other error', async () => {
        mailgunSend = new MailgunSend({ ...creds, apiKey: 'FAIL_OTHER' });
        await expect(mailgunSend.send(options))
          .rejects
          .toThrow(/failed/i);
      });
    ...
    describe('Mailgun create message parameters', () => {
      ...
      it('passes the content of the html file', async () => {
        ...
        expect(fs.readFileSync).toHaveBeenCalledWith(htmlpath, 'utf8');
        expect(mailgunSend.mailgun.messages.create).toHaveBeenCalledWith(creds.domain, {
          to: options.to,
          from: options.from,
          subject: options.subject,
